...
void loraReceive() {
  ...
  if (len == sizeof(PayloadStruct)) {
    ...
    Serial.printf("│ GPS: %.6f, %.6f | Alt: %um | Sats: %u\n",
                  lastNode.payload.lat/1e6F,
                  lastNode.payload.lon/1e6F,
                  lastNode.payload.alt,
                  lastNode.payload.sats);
    Serial.printf("│ Bat: %.1fV | RSSI: %ddBm | SNR: %.1fdB\n",
                  lastNode.payload.bat/100.0F, rssi, snr);
    Serial.println("└────────────────────────────────────────");
    ...
    // ===== PUBLICAR NO MQTT COM GPS DO GATEWAY =====
    if (isMQTTConnected()) {
      ...
      #if DEBUG_MQTT
      Serial.println("✓ Dados completos (+ GPS GW) publicados no MQTT");
      #endif
    }
  }
}
...
bool loraSendAck() {
  ...
  // Incluir GPS do gateway no ACK (se tiver fix)
  if (hasGPSFix()) {
    gwPkt.lat = (int32_t)(gwGPS.latitude * 1e6);
    gwPkt.lon = (int32_t)(gwGPS.longitude * 1e6);
    gwPkt.alt = (uint16_t)gwGPS.altitude;
    gwPkt.sats = gwGPS.satellites;
  } else {
    gwPkt.lat = 0;
    gwPkt.lon = 0;
    gwPkt.alt = 0;
    gwPkt.sats = 255;  // 255 = GW sem GPS fix
  }
  ...
  Serial.printf("📤 ACK→Node: %s", ok ? "✓" : "✗");
  if (hasGPSFix()) {
    Serial.printf(" (com GPS: %.6f, %.6f)", gwGPS.latitude, gwGPS.longitude);
  }
  Serial.println();
